...
const filter = (data, key, searchValue) => {
    return data.filter(item => item[key] === searchValue)
}
...
for( let key in conditions) {
    if(data) {
        data = filter(data, key, conditions[key])
    } else {
        data = filter(database, key, conditions[key])
    }
}
